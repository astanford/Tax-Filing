...
2. Adjustments and SE: Schedule SE (new — currently only inside what_if),
   IRA/HSA/student-loan (curated refs exist). Retirement note: the owner has
   a **legacy 401(k) with no 2025 contributions**, so no SEP/solo-401(k)
   computation is needed for the 2025 return — self-employed retirement
   remains only a possible `/tax-advisor` planning topic (the existing
   "max 401k" scenarios model W-2 deferrals and don't apply). Vehicle
   expense: the owner uses **MileIQ**, so Schedule C Line 9 / Schedule E
   Line 6 keep their existing pre-computed dollar-amount inputs; the curated
   $0.70/mile rate serves as an audit sanity check (amount ÷ rate = plausible
   miles), not an engine computation. The Form 4562 Part V trigger for
   rental auto expense still applies.
...
- Also absorbs the existing per-property Schedule E checklist
  (`docs/SCHEDULE-E-PLAN.md` §5) as structured questions, plus MileIQ
  annual-total intake (dollar amounts per activity) and a short
  retirement/contribution confirmation (IRA/HSA; 401(k) is legacy with no
  2025 contributions).
